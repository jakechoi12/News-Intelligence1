...
│  (정적 파일)   │  (수집 스크립트)   │  (자동화)                  │
├────────────────┼───────────────────┼────────────────────────────┤
│ • HTML/CSS/JS  │ • 뉴스 수집기     │ • 매일 10시(KST) 수집      │
...
1. **GitHub Actions**가 정해진 시간(한국 시간 기준 평일 오전 10시)에 자동 실행
...
* **한국 시간 기준 평일 오전 10시마다 자동 실행** (GitHub Actions cron: `0 1 * * *` UTC)
...
* 목적: 현재 물류 이슈의 핵심 키워드를 한눈에 파악
* 데이터: 최근 24시간 이내 수집된 뉴스의 키워드
* 빈도가 높을수록 크게 표시
  ...
  "sections": [
    {
      ...
      "activitySubtitle": "오전 10시 정기 수집 완료",
      "facts": [
